...
import ButtonData from 'core/widgets/ButtonData'
import HTLMData from 'core/widgets/htmlData'
...
import IFramesWidget from 'core/widgets/IFramesWidget'//mi iframe widget
import LinkexternalWidget from 'core/widgets/Link' //dsadsada
...
export default class RenderFactory extends Core {
	...
	_createLinkexternalWidget (parent, model) {
		const widget = this.$new(LinkexternalWidget);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	_createIFramesWidget(parent, model) {
		const widget = this.$new(IFramesWidget)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	...
	_createHTMLData(parent, model) {
		const widget = this.$new(HTLMData)
		widget.mode = this.mode
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	_createButtonData(parent, model) {
		const widget = this.$new(ButtonData)
		widget.mode = this.mode
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
