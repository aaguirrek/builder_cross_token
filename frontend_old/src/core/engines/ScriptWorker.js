...
async function runCode (js, qux,viewModel, console, sourceEvent) {
    ...
    if (js.indexOf('await ') > 0) {
        ...
        if (js.indexOf('JS2Code') > 0) {
            js = js.split("//ENDJS2Code")
            const AsyncFunction = Object.getPrototypeOf(async function(){}).constructor;
            const aysncJS = `return new Promise(async (resolve) => {
                ${js[0]}
                resolve()
            });`
            const aysncJS2 = `return new Promise(async (resolve) => {
                ${js[1]}
                resolve()
            });`
            console.log('ScriptWoker.runCode() > run aysnc...')
            const asycnCode = AsyncFunction('qux', 'data', 'console', 'event', aysncJS)
            await asycnCode(qux,viewModel, console, sourceEvent);
            const asycnCode2 = AsyncFunction('qux', 'data', 'console', 'event', aysncJS2)
            return asycnCode2
        }else{
