...
<template>
  <div class="MatcShareDialog" @keydown.stop @keyup.stop @keypress.stop>
    ...
    <div class="field MatcShareRow">
      <label>Publish</label>
      <input type="text" class="input" :value="publishLink" @focus="select" />
      <a class="MatcShareIcon" :href="publishLink" target="_QuantUXTest">
...
export default {
  ...
  computed: {
    ...
    publishLink() {
      let testURL = this.base + "/#/r?h=" + this.passwortedHash;
      if (this.isPublic) {
        testURL += "&log=false";
      }
      if (this.language) {
